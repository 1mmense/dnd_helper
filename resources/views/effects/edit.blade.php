<x-layout>
    <x-slot:heading>
        Редактирование эффекта: {{ $effect->name }}
    </x-slot:heading>

    <form method="POST" action="/effects/{{ $effect->id }}">
        @csrf
        @method('PATCH')
        ...
        <div class="mt-6 flex items-center justify-between gap-x-6">
            <div class="flex items-center">
                <button
                    ...
                    form="delete_form"
                    onclick="return confirm('Удалить эффект \'{{ $effect->name }}\'?')"
                >
                    Удалить
                </button>
            </div>
            ...
            <div class="flex items-center gap-x-6">
                <a href="/effects" class="text-sm/6 font-semibold text-white">
                    Отмена
                </a>
